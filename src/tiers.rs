...
//!
//! `python --tiers script.py` prints the report.
...
pub struct ChunkTiers {
    /// Which chunk this is — `main` for a whole Python program.
    pub name: String,
...
/// has gone round enough times to be recorded. pythonrs writes program output
...
///
/// The inspection compile happens *before* the run and never loads onto the
/// host, so both compiles see func/try id offsets of 0 (`rebase_program` is a
/// no-op there) and emit byte-identical ops. That is what lets fusevm's
/// op-hash-keyed caches answer for the run that just happened.
...
/// Every chunk a compiled program holds, named for the report.
///
/// A Python program is not one chunk: the module body, every `def`, every
/// comprehension/lambda body, and every arm of a `try` lower to their own
/// chunk. A hot loop usually lives in one of the latter, so reporting only
/// `main` would answer the wrong question.
fn program_chunks(prog: &crate::compiler::Program) -> Vec<(String, Chunk)> {
    ...
    for (_, f) in &prog.functions {
        out.push((format!("fn {}", f.qualname), f.chunk.clone()));
    }
    for (i, p) in prog.procs.iter().enumerate() {
        out.push((format!("proc #{i} ({})", p.name), p.chunk.clone()));
    }
    for (i, t) in prog.tries.iter().enumerate() {
        ...
        if let Some(c) = &t.finalbody {
            out.push((format!("try #{i} finally"), c.clone()));
        }
    }
    out
}
...
mod tests {
    ...
    /// Python reaches native code. A function's locals are frame slots and its
    /// `while` is emitted closing on a conditional backward branch, which is
    /// the shape fusevm's trace compiler accepts — so the loop is recorded,
    /// compiled, and installed. This is the assertion that would fail first if
    /// the loop lowering or the slot allocation regressed.
    #[test]
    fn a_function_local_counted_loop_reaches_a_compiled_trace() {
        let report = report(PROGRAM).expect("runs");
        let looped = report
            .chunks
            .iter()
        ...
        assert!(!looped.loops[0].blacklisted, "{report}");
        assert!(report.reaches_native(), "{report}");
    }
}
