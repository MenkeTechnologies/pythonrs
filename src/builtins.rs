...
fn b_mkset(vm: &mut VM, argc: u8) -> Value {
    ...
    for it in items {
        ...
        let key = host::with_instance_key(&it, host::KeyRole::Set, &cands, || {
...
pub fn call_builtin_function(
    ...
    match name {
        ...
        "hash" => {
            ...
            let k = host::with_instance_key(&v, host::KeyRole::Bare, &[], || {
                with_host(|h| h.to_key(&v))
            })?;
...
/// The literals below are the shortest decimal that round-trips to the same
/// f64 bits as CPython's full-precision digits; the value is unchanged.
...
fn dict_update(
    ...
    for (kv, v) in pairs {
        let k = host::with_instance_key(&kv, host::KeyRole::Dict, &cands, || {
            with_host(|h| h.to_key(&kv))
        })?;
...
fn set_method(recv: &Value, name: &str, args: &[Value]) -> Result<Value, String> {
    ...
    match name {
        "add" => {
            ...
            let k = host::with_instance_key(&v, host::KeyRole::Set, &cands, || {
                with_host(|h| h.to_key(&v))
            })?;
            with_host(|h| {
...
fn iter_keys_against(v: &Value, recv: &Value) -> Result<Vec<PKey>, String> {
    ...
    for it in items {
        ks.push(host::with_instance_key(
            &it,
            host::KeyRole::Set,
            &cands,
            || with_host(|h| h.to_key(&it)),
