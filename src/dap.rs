//! Debug Adapter Protocol over stdio (`python --dap`).
//!
...
//! this mode — normal runs carry zero extra ops) and run on the pure
//! interpreter (the tracing JIT would compile hot loops and skip the markers, so
...
//! comprehension, and loop bodies. Program stdout is redirected to a pipe during
//! the run and forwarded as `output` events, so `print`/`sys.stdout.write` never
//! corrupt the JSON protocol channel on the saved stdout fd.
...
/// the program actually emits a marker on that line (a blank/comment/`pass` line
/// with no compiled statement is reported unverified — a breakpoint there would
/// never fire).
...
/// program (module main + every function/lambda/comprehension body + every
/// try/except/else/finally block) — the lines on which a breakpoint can actually
/// stop.
...
/// `host::run_chunk_on`. pythonrs emits `DBG_LINE` as `Op::CallBuiltin`, not
/// `Op::Extended`, so in the current wiring the live hook is `on_debug_line`
/// (invoked from the `DBG_LINE` builtin — see the report). This shim routes an
/// `Extended(DBG_LINE)` marker to the same logic should the emission seam ever be
/// switched, and is a no-op for every other extension id.
